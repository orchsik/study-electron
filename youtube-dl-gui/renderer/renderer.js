...
async function init() {
  ...
  if (platform === "darwin") {
    ...
    $(".windowbar").prepend(
      "<img src='img/icon-titlebar-dark.png' alt='icon' class='windowbar-icon'>"
    );
  ...
  for (const option of availableOptions) {
    $("#whenDoneOptions")
      ...
      .append(`<li><a class="dropdown-item" href="#">${option}</a></li>`);
  }
  ...
  $("#subsLang").select2({
    ...
    language: { noResults: () => "No subtitles found" },
  });
  $("#autoGenSubsLang").select2({
    width: "75%",
    placeholder: "Select auto-generated subtitles",
    language: { noResults: () => "No subtitles found" },
  });
  $("#sponsorblockMark").select2({
    width: "75%",
    placeholder: "Select sections of a video to mark",
  });
  $("#sponsorblockRemove").select2({
    width: "75%",
    placeholder: "Select sections of a video to remove",
  });
  ...
  $("#infoModal .img-overlay, #infoModal .info-img")
    .on("click", () => {
      window.main.invoke("videoAction", {
        action: "downloadThumb",
        url: $("#infoModal .info-img").attr("src"),
      });
    })
    .on("mouseover", () => {
  ...
  $("#maxConcurrent").on("input", () => {
    $("#concurrentLabel").html(
      `Max concurrent jobs <strong>(${$("#maxConcurrent").val()})</strong>`
    );
  });
  ...
  $("#nameFormat").on("change", function () {
    ...
    if (value !== "custom") {
      ...
      $("#nameFormatCustom")
        .val(window.settings.nameFormat)
        .prop("disabled", false);
    }
  });
  ...
  $("#defaultConcurrent").on("click", () => {
    window.main.invoke("settingsAction", { action: "get" }).then((settings) => {
      $("#concurrentLabel").html(
        `Max concurrent jobs <strong>(${settings.defaultConcurrent})</strong>`
      );
  ...
  $("#infoModal .json").on("click", () => {
    window.main.invoke("videoAction", {
      action: "downloadInfo",
      identifier: $("#infoModal .identifier").html(),
    });
  });
  ...
  $("#subtitleBtn").on("click", () => {
    ...
    $(".video-cards")
      ...
      .each(function () {
        let state = $(this)
          .find(".subtitle-btn i")
          .hasClass("bi-card-text-strike");
        if (globalState === state) {
          if (state)
            $(this)
              .find(".subtitle-btn i")
              .removeClass("bi-card-text-strike")
              .addClass("bi-card-text")
              .attr("title", "Subtitles enabled");
          else
            $(this)
              .find(".subtitle-btn i")
              .removeClass("bi-card-text")
              .addClass("bi-card-text-strike")
              .attr("title", "Subtitles disabled");
        }
      });
    window.main.invoke("videoAction", {
      action: "globalSubtitles",
      value: globalState,
    });
    if (globalState)
      $("#subtitleBtn i")
  ...
  $("#downloadBtn").on("click", async () => {
    ...
    for (const card of videoCards) {
      let isDownloadable = await window.main.invoke("videoAction", {
        action: "downloadable",
        identifier: card.id,
      });
      if (isDownloadable) {
        if ($(card).hasClass("unified")) {
          videos.push({
            ...
            audioEncoding: $(card)
              .find(".custom-select.download-audio-encoding")
              .val(),
        ...
        $(card)
          .find(".metadata.left")
          .html("<strong>Speed: </strong>" + "0.00MiB/s")
          .show();
        $(card)
          .find(".metadata.right")
          .html("<strong>ETA: </strong>" + "Unknown")
          .show();
        $(card).find(".options").addClass("d-flex");
        $(card).find("select").addClass("d-none");
        $(card)
          .find(
            ".download-btn, .download-btn i, .subtitle-btn, .subtitle-btn i"
          )
          .addClass("disabled");
    ...
    $("#totalProgress small").html(
      `Downloading - item 0 of ${videos.length} completed`
    );
  });
  ...
  window.main.receive("binaryLock", (args) => {
    if (args.lock === true) {
      ...
      $("#add-url")
        .attr("placeholder", "Enter a video/playlist URL to add to the queue")
        .prop("disabled", false);
    }
  });
...
function toggleWhiteMode(setting) {
  ...
  $(
    ".windowbar-minimize, .windowbar-maximize, .windowbar-close > svg"
  ).toggleClass("invert", value);
  $(".windowbar > img").attr(
    "src",
    "img/icon-titlebar-" + (value ? "light" : "dark") + ".png"
  );
...
function updateEncodingDropdown(enabled) {
  $(".video-cards")
    ...
    .each(function () {
      ...
      $(this)
        .find(
          ".custom-select.download-encoding, .custom-select.download-audio-encoding"
        )
        .toggle(enabled);
    });
}
...
function updateGlobalDownloadQuality() {
  ...
  $(".video-cards")
    ...
    .each(function () {
      ...
      if ($(this).find(".custom-select.download-type").val() !== "audio") {
        $(this)
          .find(".custom-select.download-quality option.video")
          .each(function () {
  ...
  sortedFormats.sort((a, b) => {
    ...
    return (
      parseInt(bParsed.height, 10) - parseInt(aParsed.height, 10) ||
      parseInt(bParsed.fps, 10) - parseInt(aParsed.fps, 10)
    );
  });
...
async function addVideo(args) {
  ...
  if (args.type === "single") {
    $(template).find(".card-title").html(args.title).prop("title", args.title);
    $(template)
      .find(".progress-bar")
    ...
    $(template)
      ...
      .html(
        "<strong>Duration: </strong>" +
          (args.duration == null ? "Unknown" : args.duration)
      );
    ...
    if (!args.hasFilesizes) {
      $(template)
        .find(".metadata.right")
        .html("<strong>Size: </strong>Unknown");
    } else if (args.loadSize) {
      $(template)
        .find(".metadata.right")
        .html('<strong>Size: </strong><i class="lds-dual-ring"></i>');
    } else {
      $(template)
        .find(".metadata.right")
        .html(
          '<strong>Size: </strong><button class="btn btn-dark">Load</button>'
        );
    }

    $(template)
      .find(".custom-select.download-type")
      .on("change", function () {
        ...
        if (args.formats.length > 0) {
          $(template)
            .find(".custom-select.download-quality")
            .val(
              isAudio
                ? "best"
                : args.formats[args.selected_format_index].display_name
            )
            .change();
        }
      });
    ...
    if (args.formats.length === 0) {
      $(template)
        .find(".custom-select.download-quality")
        .append(new Option("No formats", "", true))
        .prop("disabled", true);
    ...
    $(template)
      .find(".remove-btn")
      .on("click", () => removeVideo(getCard(args.identifier)));
    ...
    $(template)
      ...
      .on("click", () => {
        ...
        $(template)
          .find(
            ".download-btn i, .download-btn, .subtitle-btn, .subtitle-btn i"
          )
          .addClass("disabled");
    ...
    $(template).find(".card-title").html(args.url).prop("title", args.url);
    $(template)
      .find(".progress-bar")
      .addClass("progress-bar-striped")
      .addClass("progress-bar-animated")
...
function removeVideo(card) {
  ...
  if (
    ...
    $(card).find(".custom-select.download-type").is(":visible") ||
    $(card).find(".btn.btn-dark.folder").is(":visible") ||
    $(card).find(".row.error.d-none").is(":visible") ||
    $(card).find(".url").length
  ) {
    $(btn).popover("hide");
    $(card).remove();
    window.main.invoke("videoAction", {
      action: "stop",
      identifier: $(card).prop("id"),
    });
  } else {
    $(btn).popover("show");
...
async function setUnifiedPlaylist(args) {
  ...
  $(card)
    .find(".metadata.right")
    .html("<strong>Playlist size: </strong>" + args.length);
  $(card)
    .find(".metadata.left")
    .html(
      "<strong>Uploader: </strong>" +
        (args.uploader == null ? "Unknown" : args.uploader)
    );
  if (window.settings.enableEncoding) {
  ...
  $(card).find(".card-title").html(args.title).prop("title", args.title);
  $(card)
    .find(".progress-bar")
    .addClass("progress-bar-striped")
    .addClass("progress-bar-animated")
  ...
  $(card)
    .find(
      ".download-btn i, .download-btn, .subtitle-btn, .subtitle-btn i, .remove-btn i, .remove-btn"
    )
  ...
  $(card)
    ...
    .on("change", function () {
      ...
      $(card)
        .find(".custom-select.download-quality")
        .val(isAudio ? "best" : args.formats[0].display_name)
        .change();
    });
  ...
  $(card)
    ...
    .on("click", () => {
      ...
      $(card).find("select").addClass("d-none");
      $(card)
        .find(".download-btn i, .download-btn, .subtitle-btn, .subtitle-btn i")
        .addClass("disabled");
    });
...
function updateCodecs(card, newValue) {
  const encodingValue = $(card)
    .find(".custom-select.download-encoding :selected")
    .val();
...
function updateProgress(args) {
  ...
  if (args.progress.finished != null && args.progress.finished) {
    if (args.progress.isPlaylist) {
      ...
      if (args.progress.isAudio == null)
        $(card).find(".progress small").html("Item downloaded - 100%");
      else
        $(card)
          .find(".progress small")
          .html(
            (args.progress.isAudio ? "Audio" : "Video") + " downloaded - 100%"
          );
      $(card)
        .find(".progress-bar")
        .attr("aria-valuenow", 100)
      ...
      $(card).find(".progress-bar").removeClass("progress-bar-striped");
      $(card).find(".open").addClass("d-flex");
      if (window.settings.nameFormatMode === "custom")
        $(card).find(".open .item").prop("disabled", true);
    }
  ...
  if (args.progress.done != null && args.progress.total != null) {
    ...
    $(card)
      .find(".progress-bar")
      .attr("aria-valuenow", args.progress.percentage.slice(0, -1))
      .css("width", args.progress.percentage);
    $(card)
      .find(".progress small")
      .html(
        `${args.progress.percentage} - ${args.progress.done} of ${args.progress.total} `
      );
  } else if (args.progress.percentage != null) {
    if (
      parseFloat(args.progress.percentage.slice(0, -1)) >
      parseFloat($(card).find(".progress-bar").attr("aria-valuenow"))
    ) {
      $(card)
        .find(".progress-bar")
        .attr("aria-valuenow", args.progress.percentage.slice(0, -1))
      ...
      if (args.progress.percentage.slice(0, -1) === "100.0") {
        ...
        if (args.progress.isAudio == null)
          $(card)
            .find(".progress small")
            .html("Downloading item - " + args.progress.percentage);
        else
          $(card)
            .find(".progress small")
            .html(
              (args.progress.isAudio
                ? "Downloading audio"
                : "Downloading video") +
                " - " +
                args.progress.percentage
            );
      }
      if (!progressCooldown.includes(args.identifier)) {
        ...
        $(card)
          .find(".metadata.right")
          .html("<strong>ETA: </strong>" + args.progress.eta)
          .show();
        $(card)
          .find(".metadata.left")
          .html("<strong>Speed: </strong>" + args.progress.speed)
          .show();
        setTimeout(() => {
          progressCooldown = progressCooldown.filter(
...
function updateTotalProgress(args) {
  ...
  $("#totalProgress small").html(
    `Downloading - item ${args.progress.done} of ${args.progress.total} completed`
  );
...
function updateSize(identifier, clicked) {
  ...
  if (
    $(card).find(".custom-select.download-quality").prop("disabled") === true
  ) {
    sizeCooldown = sizeCooldown.filter((item) => item !== identifier);
    $(card)
      .find(".metadata.right")
  ...
  window.main
    .invoke("videoAction", {
      ...
      videoOnly:
        $(card).find(".custom-select.download-type").val() === "videoOnly",
      clicked: clicked,
    })
    .then((size) => {
      if (size != null && size === "Unknown") {
        ...
        if (
          $(card).find(".custom-select.download-quality").val() === formatLabel
        ) {
          sizeCache = sizeCache.filter((item) => item[0] !== identifier);
          sizeCache.push([identifier, size]);
          $(card)
            .find(".metadata.right")
            .html("<strong>Size: </strong>" + convertBytes(size));
        }
      } else {
        $(card)
          .find(".metadata.right")
...
async function updateVideoSettings(identifier) {
  ...
  if (qualityValue === "best") {
    $(card)
      ...
      .val(
        $(card)
          .find(`.custom-select.download-quality option.${classValue}:first`)
          .val()
      );
    ...
    if (isAudio) {
      ...
      $(card)
        ...
        .val(
          $(card)
            .find(`.custom-select.download-quality option.${classValue}:last`)
            .val()
        );
    }
  } else if (!isAudio) {
    ...
    if (formats.includes(qualityValue)) {
      ...
      const closest = formats.reduce((a, b) => {
        ...
        return Math.abs(parsedB.height - search.height) <
          Math.abs(parsedA.height - search.height)
          ? b
          : a;
      });
  ...
  if (
    oldQuality != null &&
    oldType != null &&
    (oldQuality !== $(card).find(".custom-select.download-quality").val() ||
      oldType !== $(card).find(".custom-select.download-type").val())
  ) {
...
function disableEncodingDropdowns(typeValue, card) {
  ...
  $(card)
    .find(".custom-select.download-encoding")
    .prop("disabled", isAudio && !isVideoOnly);
  $(card)
    .find(".custom-select.download-audio-encoding")
    .prop("disabled", !isAudio && isVideoOnly);
}
...
async function getSettings() {
  const settings = await window.main.invoke("settingsAction", {
    action: "get",
  });
  ...
  $("#nameFormatCustom")
    .val(settings.nameFormat)
    .prop("disabled", settings.nameFormatMode === "custom");
  ...
  $("#concurrentLabel").html(
    `Max concurrent jobs <strong>(${settings.maxConcurrent})</strong>`
  );
...
async function updateTotalSize() {
  ...
  if (total > 0)
    $("#totalProgress small").html(
      "Ready to download! - Total queried size: " + convertBytes(total)
    );
...
function saveSubtitlesModal() {
  ...
  const autoGen = $("#autoGenSubsLang")
    .select2("data")
    .map((option) => option.id);
  if ($(modal).find("#enableSubs").is(":checked"))
    $(card)
      .find(".subtitle-btn i")
      .removeClass("bi-card-text-strike")
      .addClass("bi-card-text")
  ...
  window.main.invoke("videoAction", {
    ...
    subs: subs,
    autoGen: autoGen,
    enabled: $(modal).find("#enableSubs").prop("checked"),
    unified: $(card).hasClass("unified"),
  });
}

async function showSubtitleModal(identifier, card) {
  const modal = $("#subsModal");
  const availableLangs = await window.main.invoke("getSubtitles", {
    identifier: identifier,
    unified: $(card).hasClass("unified"),
  });
  const selectedLangs = await window.main.invoke("getSelectedSubtitles", {
    identifier: identifier,
  });
  if ($(modal).find(".identifier").length) {
    ...
    $(modal).append(
      `<input type="hidden" class="identifier" value="${identifier}">`
    );
  }
  ...
  if (availableLangs[0].length === 0 && availableLangs[1].length === 0) {
    ...
    if ($(card).hasClass("unified")) {
      $(modal)
        .find(".description")
        .html("Select the subtitle languages you want to try to download.");
    } else {
      $(modal)
        .find(".description")
        .text("Select the subtitle languages you want to download.");
    }
...
function showInfoModal(info, identifier) {
  ...
  if (data == null) {
    ...
    data = {
      ...
      description:
        "This video threw an error. Not all info is available.\n\nError:\n" +
  ...
  $(modal)
    ...
    .html(
      data.description == null ? "No description was found." : data.description
    );
  $(modal)
    .find(".uploader")
    .html(
      "<strong>Uploader: </strong>" +
        (data.uploader == null ? "Unknown" : data.uploader)
    );
  $(modal)
    .find(".extractor")
    .html(
      "<strong>Extractor: </strong>" +
        (data.extractor == null ? "Unknown" : data.extractor)
    );
  $(modal)
    .find(".url")
    .html(
  ...
  $(modal)
    .find('[title="Like / dislikes"]')
    .html(
      '<i class="bi bi-hand-thumbs-up"></i> ' +
        (data.like_count == null ? "-" : data.like_count) +
        ' &nbsp;&nbsp; <i class="bi bi-hand-thumbs-down"></i> ' +
        (info.dislike_count == null ? "-" : info.dislike_count)
    );
  $(modal)
    .find('[title="Average rating"]')
    .html(
      '<i class="bi bi-star"></i> ' +
        (data.average_rating == null
          ? "-"
          : data.average_rating.toString().slice(0, 3))
    );
  $(modal)
    .find('[title="Duration"]')
    .html(
...
function changeDownloadIconToLog(card) {
  ...
  $(card).find(".download-btn").unbind().removeClass("disabled");
}
    ...
    if (log == null) {
      ...
      for (const line of log) {
        if (line.startsWith("WARNING")) {
          const pre =
            line.slice(0, line.indexOf("W")) +
            "<strong>" +
            line.slice(line.indexOf("W"));
          const suffixed =
            pre.slice(0, pre.indexOf(":") + 1) +
            "</strong>" +
            pre.slice(pre.indexOf(":") + 1);
          fullLog += "<p class='text-warning'>" + suffixed + "</p>";
        } else if (line.startsWith("ERROR")) {
          const pre =
            line.slice(0, line.indexOf("E")) +
            "<strong>" +
            line.slice(line.indexOf("E"));
          const suffixed =
            pre.slice(0, pre.indexOf(":") + 1) +
            "</strong>" +
            pre.slice(pre.indexOf(":") + 1);
...
function setError(code, description, unexpected, identifier, url) {
  ...
  $(card)
    ...
    .removeClass("progress-bar-animated")
    .css("width", "100%")
    .css("background-color", "var(--error-color)");
  $(card)
    .find(".buttons")
    .children()
    .each(function () {
      if (
        $(this).hasClass("remove-btn") ||
        $(this).hasClass("info-btn") ||
        $(this).find("i").hasClass("bi-journal-text")
      ) {
  ...
  $(card)
    .css("box-shadow", "none")
    .css("border", "solid 1px var(--error-color)");
  $(card)
    .find(".progress small")
    .html("Error! " + code + ".");
  $(card).find(".progress").addClass("d-flex");
      ...
      .on("click", () => {
        window.main
          .invoke("errorReport", {
            identifier: identifier,
            type: $(card).find(".custom-select.download-type").val(),
            quality: $(card).find(".custom-select.download-quality").val(),
          })
          .then((id) => {
            $(card)
              .find(".progress small")
              .html("Error reported! Report ID: " + id);
