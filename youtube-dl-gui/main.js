...
function startCriticalHandlers(env) {
  ...
  if (env.settings.updateBinary) {
    ...
    win.webContents.send('binaryLock', {
      lock: true,
      placeholder: `Checking for a new version of ffmpeg...`,
    });
    ffmpegUpdater.checkUpdate().finally(() => {
      win.webContents.send('binaryLock', {
        lock: true,
        placeholder: `Checking for a new version of yt-dlp...`,
      });
  ...
  win.webContents.send('videoAction', {
    action: 'setDownloadType',
    type: env.settings.downloadType,
  });
  ...
  if (appStarting) {
    ...
    if (
      !env.paths.appPath.includes('\\AppData\\Local\\Temp\\') &&
      !env.paths.appPath.includes('WindowsApps')
    ) {
    ...
    ipcMain.handle('videoAction', async (event, args) => {
      switch (args.action) {
        ...
        case 'download':
          ...
          else if (args.downloadType === 'unified')
            queryManager.downloadUnifiedPlaylist(args);
          else if (args.downloadType === 'single')
        ...
        case 'downloadThumb':
          ...
          break;
        case 'getSize':
          return await queryManager.getSize(
            args.identifier,
            args.formatLabel,
            args.audioOnly,
            args.videoOnly,
...
ipcMain.handle('cookieFile', async (event, clear) => {
  ...
  let result = await dialog.showOpenDialog(win, {
    ...
    defaultPath:
      env.settings.cookiePath != null
        ? env.settings.cookiePath
        : env.settings.downloadPath,
...
ipcMain.handle('messageBox', (event, args) => {
  dialog.showMessageBoxSync(win, {
    title: args.title,
    message: args.message.startsWith('Youtube-dl returned an empty object')
      ? 'Youtube-dl returned an empty object'
      : args.message,
    type: 'none',
    buttons: [],
  });
