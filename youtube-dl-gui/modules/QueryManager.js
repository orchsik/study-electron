...
class QueryManager {
  ...
  async manage(url) {
    ...
    const initialQuery = await new InfoQuery(
      url,
      metadataVideo.identifier,
      this.environment
    ).connect();
    if (metadataVideo.error) return;
    if (Utils.isYouTubeChannel(url)) {
      const actualQuery = await new InfoQuery(
        initialQuery.entries[0].url,
        metadataVideo.identifier,
        this.environment
      ).connect();
      if (metadataVideo.error) return;
      this.removeVideo(metadataVideo);
      if (actualQuery.entries == null || actualQuery.entries.length === 0)
    ...
    switch (Utils.detectInfoType(initialQuery)) {
      ...
      case 'playlist':
        ...
        break;
      case 'livestream':
        this.environment.errorHandler.raiseError(
          {
            code: 'Not supported',
            description: 'Livestreams are not yet supported.',
          },
          metadataVideo.identifier
        );
        break;
  ...
  managePlaylist(initialQuery, url) {
    this.playlistMetadata = this.playlistMetadata.concat(
      Utils.generatePlaylistMetadata(initialQuery)
    );
    let playlistVideo = new Video(url, 'playlist', this.environment);
    this.addVideo(playlistVideo);
    const playlistQuery = new InfoQueryList(
      initialQuery,
      this.environment,
      new ProgressBar(this, playlistVideo)
    );
      ...
      if (videos.length > this.environment.settings.splitMode) {
        ...
        for (const video of videos) {
          ...
          for (const format of video.formats) {
            format.display_name = Format.getDisplayName(
              format.height,
              format.fps
            );
        ...
        totalFormats = totalFormats.filter(
          (v, i, a) =>
            a.findIndex((t) => t.height === v.height && t.fps === v.fps) === i
        );
        ...
        const uploader =
          initialQuery.uploader == null ? 'Unknown' : initialQuery.uploader;
        if (this.window != null) {
  ...
  addVideo(video) {
    ...
    let args = {
      ...
      formats: formats,
      selected_format_index: video.hasMetadata
        ? video.selected_format_index
        : null,
  ...
  downloadVideo(args) {
    ...
    if (!downloadVideo.audioOnly) {
      for (const format of downloadVideo.formats) {
        if (format.getDisplayName() === args.format) {
          downloadVideo.selected_format_index =
            downloadVideo.formats.indexOf(format);
          break;
        }
      }
    }
    downloadVideo.audioQuality =
      downloadVideo.audioQuality != null ? downloadVideo.audioQuality : 'best';
    let progressBar = new ProgressBar(this, downloadVideo);
    downloadVideo.setQuery(
      new DownloadQuery(
        downloadVideo.url,
        downloadVideo,
        this.environment,
    ...
    downloadVideo.query.connect().then(() => {
      ...
      if (this.environment.settings.downloadJsonMetadata)
        this.saveInfo(downloadVideo.identifier, false);
  ...
  downloadAllVideos(args) {
    ...
    for (const videoObj of args.videos) {
      ...
      if (video.videos == null) {
        ...
        const videoMeta = Utils.getVideoInPlaylistMetadata(
          video.url,
          null,
          this.playlistMetadata
        );
        ...
        video.audioQuality =
          video.audioQuality != null ? video.audioQuality : 'best';
        videosToDownload.push(video);
      } else {
        video.url = videoObj.url;
        unifiedPlaylists.push(video);
        this.getUnifiedVideos(
          video,
          video.videos,
          videoObj.type === 'audio',
          videoObj.format,
          videoObj.downloadSubs
        );
    ...
    let downloadList = new DownloadQueryList(
      videosToDownload,
      videoMetadata,
      this.environment,
      this,
    ...
    for (const unifiedPlaylist of unifiedPlaylists) {
      unifiedPlaylist.setQuery(downloadList);
    }
    downloadList.start().then(() => {
      for (const unifiedPlaylist of unifiedPlaylists) {
        unifiedPlaylist.downloaded = true;
      }
      this.updateGlobalButtons();
    ...
    if (!playlist.audioOnly) {
      for (const video of videos) {
        ...
        if (!gotFormatMatch) {
          ...
          const output = video.formats.reduce((prev, curr) =>
            Math.abs(curr.height - suppliedFormat.height) <
            Math.abs(prev.height - suppliedFormat.height)
              ? curr
              : prev
          );
      ...
      for (const video of videos) {
        ...
        video.audioQuality =
          playlist.audioQuality != null ? playlist.audioQuality : 'best';
      }
    }
  ...
  downloadUnifiedPlaylist(args) {
    ...
    const metadata = videos
      .map((vid) =>
        Utils.getVideoInPlaylistMetadata(
          vid.url,
          playlist.url,
          this.playlistMetadata
        )
  ...
  async getSize(
    ...
    if (cachedSize != null) {
      ...
      if (!clicked && this.environment.settings.sizeMode === 'click') {
        ...
        return await this.querySize(
          video,
          formatLabel,
          video.getFormatFromLabel(formatLabel),
          audioOnly,
          videoOnly
        );
      }
  ...
  async querySize(video, formatLabel, format, audioOnly, videoOnly) {
    const sizeQuery = new SizeQuery(
      video,
      audioOnly,
      videoOnly,
      audioOnly ? formatLabel : format,
      this.environment
    );
  ...
  getCachedSize(video, formatLabel, audioOnly, videoOnly) {
    if (audioOnly) {
      ...
      const cachedFormat = video.videoOnlySizeCache.find(
        (format) => format.getDisplayName() === formatLabel
      );
  ...
  removeVideo(video) {
    this.managedVideos = this.managedVideos.filter(
      (item) => item.identifier !== video.identifier
    );
    this.playlistMetadata = this.playlistMetadata.filter(
      (item) => item.video_url !== video.url
    );
    this.window.webContents.send('videoAction', {
      action: 'remove',
      identifier: video.identifier,
  ...
  updateProgress(video, progress_args) {
    ...
    try {
      ...
      console.log(
        'Blocked webContents IPC call, the window object was destroyed.'
      );
    }
  }
    ...
    if (file == null) {
      fs.readdir(video.downloadedPath, (err, files) => {
        for (const searchFile of files) {
          if (
            searchFile.substr(0, searchFile.lastIndexOf('.')) ===
            video.getFilename()
          ) {
            file = searchFile;
            break;
  ...
  getUnifiedAvailableSubtitles(videos) {
    ...
    for (const video of videos) {
      if (video.subtitles != null && video.subtitles.length !== 0) {
        totalSubs = totalSubs.concat(
          Object.keys(video.subtitles).map((sub) => {
            return { iso: sub, name: Utils.getNameFromISO(sub) };
          })
        );
      }
      if (video.autoCaptions != null && video.autoCaptions.length !== 0) {
        totalAutoGen = totalAutoGen.concat(
          Object.keys(video.autoCaptions).map((sub) => {
            return { iso: sub, name: Utils.getNameFromISO(sub) };
          })
        );
      }
    ...
    return [
      totalSubsDedupe.sort(Utils.sortSubtitles),
      totalAutoGenDedupe.sort(Utils.sortSubtitles),
    ];
  }
  ...
  async saveInfo(infoVideo, askPath = true) {
    ...
    if (!result.canceled) {
      fs.writeFileSync(
        result.filePath,
        JSON.stringify(video.serialize(), null, 3)
      );
    }
  }
  ...
  async saveThumb(link) {
    let result = await dialog.showSaveDialog(this.window, {
      defaultPath: path.join(
        this.environment.settings.downloadPath,
        'thumb_' + path.basename(url.parse(link).pathname)
      ),
  ...
  isDownloadable(video) {
    ...
    return !(
      usedVideo == null ||
      usedVideo.type !== 'single' ||
      usedVideo.error ||
      usedVideo.downloaded
    );
  }
  ...
  getTaskList() {
    ...
    for (const video of this.playlistMetadata) {
      for (let i = 0; i < urlList.length; i++) {
        if (
          urlList[i] === video.video_url ||
          urlList[i] === video.playlist_url
        ) {
          urlList.splice(i, 1);
          i--;
