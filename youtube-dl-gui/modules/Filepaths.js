...
class Filepaths {
  ...
  async generateFilepaths() {
    switch (this.platform) {
      case 'win32':
        this.unpackedPrefix = path.join(
          path.dirname(this.appPath),
          'app.asar.unpacked'
        );
        this.packedPrefix = this.appPath;
        this.ffmpeg = this.app.isPackaged
          ? path.join(this.unpackedPrefix, 'binaries')
          : 'binaries';
        this.ytdl = this.app.isPackaged
          ? path.join(this.unpackedPrefix, 'binaries/yt-dlp.exe')
          : 'binaries/yt-dlp.exe';
        this.icon = this.app.isPackaged
          ? path.join(this.packedPrefix, 'renderer/img/icon.png')
          : 'renderer/img/icon.png';
        this.settings = this.app.isPackaged
          ? path.join(this.unpackedPrefix, 'userSettings')
          : 'userSettings';
        this.taskList = this.app.isPackaged
          ? path.join(this.unpackedPrefix, 'taskList')
          : 'taskList';
        this.ytdlVersion = this.app.isPackaged
          ? path.join(this.unpackedPrefix, 'binaries/ytdlVersion')
          : 'binaries/ytdlVersion';
        this.ffmpegVersion = this.app.isPackaged
          ? path.join(this.unpackedPrefix, 'binaries/ffmpegVersion')
          : 'binaries/ffmpegVersion';
        break;
      case 'win32app': {
        const appDir = path
          .basename(path.join(this.appPath, '../../..'))
          .replace(/_(.*)_/g, '_');
        this.binaryPath = path.join(
          this.app.getPath('home'),
          'AppData/Local/Packages/' +
            appDir +
            '/LocalCache/Roaming/open-video-downloader-app'
        );
      ...
      case 'darwin':
        ...
        this.ffmpeg = this.app.isPackaged
          ...
          : 'binaries';
        this.ytdl = this.app.isPackaged
          ? path.join(this.unpackedPrefix, this.getMacOSPathYtDlp())
          : this.getMacOSPathYtDlp();
        this.icon = this.app.isPackaged
          ? path.join(this.packedPrefix, 'renderer/img/icon.png')
          : 'renderer/img/icon.png';
        this.settings = this.app.isPackaged
      ...
      case 'linux':
        ...
        this.ytdl = this.app.isPackaged
          ? path.join(this.persistentPath, 'yt-dlp-unix')
        ...
        this.icon = this.app.isPackaged
          ...
          : 'renderer/img/icon.png';
        this.settings = this.app.isPackaged
          ? path.join(this.persistentPath, 'userSettings')
          : 'userSettings';
        this.taskList = this.app.isPackaged
          ? path.join(this.persistentPath, 'taskList')
          : 'taskList';
        this.ytdlVersion = this.app.isPackaged
          ? path.join(this.persistentPath, 'ytdlVersion')
          : 'binaries/ytdlVersion';
        this.ffmpegVersion = this.app.isPackaged
          ? path.join(this.persistentPath, 'ffmpegVersion')
          : 'binaries/ffmpegVersion';
        this.setPermissions();
        break;
    }
  ...
  async validateDownloadPath() {
    ...
    try {
      ...
      console.warn(
        'The configured download path could not be found, switching to downloads folder.'
      );
  ...
  setDefaultDownloadPath() {
    try {
      ...
      console.warn(
        'Using home path as download location, as downloads was not found.'
      );
  ...
  async removeLeftOver() {
    const filename =
      process.platform === 'win32' ? 'youtube-dl.exe' : 'youtube-dl-unix';
