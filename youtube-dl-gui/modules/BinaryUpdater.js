  ...
  async checkUpdate() {
    ...
    if (remoteVersion === localVersion) {
      ...
      this.win.webContents.send('binaryLock', {
        lock: true,
        placeholder: `Installing yt-dlp version: ${remoteVersion}. Preparing...`,
      });
      ...
      this.win.webContents.send('binaryLock', {
        lock: true,
        placeholder: `Updating yt-dlp to version: ${remoteVersion}. Preparing...`,
      });
      await this.downloadUpdate(remoteUrl, remoteVersion);
      this.paths.setPermissions();
    }
  ...
  async getRemoteVersion() {
    ...
    try {
      ...
      if (err.response == null) {
        console.error(
          'An error occurred while retrieving the latest yt-dlp version data.'
        );
        return null;
      }
      if (res.status === 302) {
  ...
  async downloadUpdate(remoteUrl, remoteVersion) {
    ...
    return await new Promise((resolve, reject) => {
      ...
      data.on('data', (chunk) => {
        ...
        this.win.webContents.send('binaryLock', {
          lock: true,
          placeholder: `${this.action} yt-dlp ${remoteVersion} - ${percentage} of ${total}`,
        });
      });
      writer.on('error', (err) => {
        error = err;
