...
class AppUpdater {
  ...
  initializeEvents() {
    autoUpdater.on('download-progress', (progressObj) => {
      ...
      const progress = `${Utils.convertBytes(
        progressObj.transferred
      )} / ${Utils.convertBytes(progressObj.total)}`;
  ...
  isUpdateAllowed() {
    return (
      (this.env.settings.updateApplication && process.argv[2] !== '--dev') ||
      (process.argv[2] === '--dev' && process.argv[3] === '--test-update')
    );
  }
