...
class ErrorHandler {
  constructor(win, queryManager, env) {
    ...
    this.loadErrorDefinitions().then(
      (errorDefs) => (this.errorDefinitions = errorDefs)
    );
  }
      ...
      if (Array.isArray(errorDef.trigger)) {
        for (const trigger of errorDef.trigger) {
          if (stderr.includes(trigger)) {
            if (
              errorDef.code === 'ffmpeg not found' &&
              process.argv[2] === '--dev'
            )
              return false; //Do not raise a 'ffmpeg not found' error when in dev mode
            if (errorDef.code === 'Thumbnail embedding not supported')
              return false; //Do not raise an error when thumbnails can't be embedded due to unsupported container
            foundError = true;
        ...
        if (errorDef.code === 'ffmpeg not found' && process.argv[2] === '--dev')
          return false; //Do not raise a 'ffmpeg not found' error when in dev mode
  ...
  raiseUnhandledError(code, error, identifier) {
    ...
    Sentry.captureMessage(error === code ? error : code, (scope) => {
      ...
      if (video.selected_format_index != null) {
        scope.setContext(
          'selected_format',
          video.formats[video.selected_format_index].serialize()
        );
      }
  ...
  raiseError(errorDef, identifier) {
    ...
    this.win.webContents.send('error', {
      error: errorDef,
      identifier: identifier,
      unexpected: false,
      url: video.url,
