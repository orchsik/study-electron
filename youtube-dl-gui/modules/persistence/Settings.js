...
class Settings {
  constructor(
    ...
    this.audioOutputFormat =
      audioOutputFormat == null ? 'none' : audioOutputFormat;
    this.downloadPath =
      downloadPath == null ? env.app.getPath('downloads') : downloadPath;
    this.proxy = proxy == null ? '' : proxy;
    this.rateLimit = rateLimit == null ? '' : rateLimit;
    this.autoFillClipboard =
      autoFillClipboard == null ? true : autoFillClipboard;
    ...
    this.validateCertificate =
      validateCertificate == null ? false : validateCertificate;
    ...
    this.sponsorblockRemove =
      sponsorblockRemove == null ? '' : sponsorblockRemove;
    this.sponsorblockApi =
      sponsorblockApi == null ? 'https://sponsor.ajay.app' : sponsorblockApi;
    this.downloadMetadata = downloadMetadata == null ? true : downloadMetadata;
    this.downloadJsonMetadata =
      downloadJsonMetadata == null ? false : downloadJsonMetadata;
    this.downloadThumbnail =
      downloadThumbnail == null ? false : downloadThumbnail;
    this.keepUnmerged = keepUnmerged == null ? false : keepUnmerged;
    this.calculateTotalSize =
      calculateTotalSize == null ? true : calculateTotalSize;
    this.sizeMode = sizeMode == null ? 'click' : sizeMode;
    this.splitMode = splitMode == null ? '49' : splitMode;
    this.maxConcurrent =
      maxConcurrent == null || maxConcurrent <= 0
        ? this.getDefaultMaxConcurrent()
        : maxConcurrent; //Max concurrent is standard half of the system's available cores
    this.updateBinary = updateBinary == null ? true : updateBinary;
    this.downloadType = downloadType == null ? 'video' : downloadType;
    this.updateApplication =
      updateApplication == null ? true : updateApplication;
