...
class Video {
  ...
  getFilename() {
    if (this.hasMetadata) {
      ...
      if (this.formats.length === 0) {
        ...
        let fps =
          this.formats[this.selected_format_index].fps != null
            ? this.formats[this.selected_format_index].fps
            : '';
        let height = this.formats[this.selected_format_index].height;
        if (
          this.environment.settings.nameFormatMode ===
          '%(title).200s-(%(height)sp%(fps).0d).%(ext)s'
        ) {
          return (
            this.title.substr(0, 200) +
            '-(' +
            height +
            'p' +
            fps.toString().substr(0, 2) +
            ')'
  ...
  setMetadata(metadata) {
    ...
    if (metadata.duration != null)
      this.duration = new Date(metadata.duration * 1000)
        .toISOString()
        .substr(11, 8);
    if (this.duration != null && this.duration.split(':')[0] === '00')
      this.duration = this.duration.substr(3);
