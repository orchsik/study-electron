...
class Query {
  ...
  async start(url, args, cb) {
    ...
    if (
      this.environment.settings.proxy != null &&
      this.environment.settings.proxy.length > 0
    ) {
      args.push('--proxy');
    ...
    if (this.environment.pythonCommand !== 'python') {
      //If standard python is not available use another install if detected
      ...
      try {
        ...
        if (
          ...
          if (
            ...
            this.environment.errorHandler.raiseUnhandledError(
              'Unhandled error (execa)',
