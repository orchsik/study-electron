  ...
  async connect() {
    ...
    if (this.video.audioOnly) {
      ...
      if (this.video.selectedAudioEncoding !== 'none') {
        args.push('-f');
        args.push(
          'bestaudio[acodec=' + this.video.selectedAudioEncoding + ']/bestaudio'
        );
      ...
      if (this.video.formats.length !== 0) {
        let format;
        const encoding =
          this.video.selectedEncoding === 'none'
            ? ''
            : '[vcodec=' + this.video.selectedEncoding + ']';
        const audioEncoding =
          this.video.selectedAudioEncoding === 'none'
            ? ''
            : '[acodec=' + this.video.selectedAudioEncoding + ']';
  ...
  async removeThumbnail(extension) {
    ...
    if (filename != null) {
      ...
      try {
        ...
        console.log(
          'No left-over thumbnail found to remove. (' + filenameExt + ')'
        );
        if (extension !== '.webp') {
