  ...
  async start() {
    return await new Promise((resolve) => {
      for (let video of this.videos) {
        ...
        if (
          video.parentID != null &&
          !this.parentProgress.some((e) => e.id === video.parentID)
        ) {
        ...
        video.query.connect().then((returnValue) => {
          if (video.parentID != null) {
            const progress = this.parentProgress.find(
              (e) => e.id === video.parentID
            );
            if (returnValue === 'killed' || returnValue !== 'done')
              progress.cancelled++;
            progress.done++;
            ...
            this.done++;
            progress.bar.updatePlaylist(
              progress.done - progress.cancelled,
              progress.length - progress.cancelled
            );
          ...
          this.progressBar.updatePlaylist(
            this.done - this.cancelled,
            this.length - this.cancelled
          );
          if (!video.error) {
            if (this.environment.settings.downloadJsonMetadata)
              this.manager.saveInfo(video, false);
