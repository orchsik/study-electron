...
class FfmpegUpdater {
  ...
  async checkUpdate() {
    if (await this.checkPreInstalled()) {
      console.log(
        'FFmpeg and FFprobe already installed, skipping auto-install.'
      );
      return;
    }
    ...
    const { remoteFfmpegUrl, remoteFfprobeUrl, remoteVersion } =
    ...
    if (remoteVersion === localVersion) {
      ...
      this.win.webContents.send('binaryLock', {
        lock: true,
        placeholder: `Installing ffmpeg version: ${remoteVersion}. Preparing...`,
      });
      await this.downloadUpdate(
        remoteFfmpegUrl,
        remoteVersion,
        'ffmpeg' + this.getFileExtension()
      );
      this.win.webContents.send('binaryLock', {
        lock: true,
        placeholder: `Installing ffprobe version: ${remoteVersion}. Preparing...`,
      });
      await this.downloadUpdate(
        remoteFfprobeUrl,
        remoteVersion,
        'ffprobe' + this.getFileExtension()
      );
      await this.writeVersionInfo(remoteVersion);
    } else if (remoteVersion == null) {
      transaction.setTag('download', 'down');
      console.log(
        'Unable to check for new updates, ffbinaries.com may be down.'
      );
    } else {
      ...
      this.win.webContents.send('binaryLock', {
        lock: true,
        placeholder: `Updating ffmpeg to version: ${remoteVersion}. Preparing...`,
      });
      await this.downloadUpdate(
        remoteFfmpegUrl,
        remoteVersion,
      ...
      this.win.webContents.send('binaryLock', {
        lock: true,
        placeholder: `Updating ffprobe to version: ${remoteVersion}. Preparing...`,
      });
      await this.downloadUpdate(
        remoteFfprobeUrl,
        remoteVersion,
  ...
  async getLocalVersion() {
    ...
    try {
      await fs.promises.access(
        path.join(this.paths.ffmpeg, 'ffmpeg' + this.getFileExtension())
      );
      await fs.promises.access(
        path.join(this.paths.ffmpeg, 'ffprobe' + this.getFileExtension())
      );
    } catch (e) {
      data = null;
    }
  ...
  async downloadUpdate(url, version, filename) {
    ...
    await new Promise((resolve, reject) => {
      ...
      data.on('data', (chunk) => {
        ...
        this.win.webContents.send('binaryLock', {
          lock: true,
          placeholder: `${this.action} ${artifact} ${version} - ${percentage} of ${total}`,
        });
      });
      writer.on('error', (err) => {
        error = err;
    ...
    this.win.webContents.send('binaryLock', {
      lock: true,
      placeholder: `${this.action} ${artifact} ${version} - Extracting binaries...`,
    });
    ...
    fs.rmdirSync(path.join(this.paths.ffmpeg, 'downloads'), {
      recursive: true,
      force: true,
    });
  }
